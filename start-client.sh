...
# print usage
usage() {
    echo "Usage: $0 [-r|--repository <REPOSITORY>] [-p|--prefix <IMAGE_PREFIX>] [--remote-host <ADDRESS>] [-c <COMMAND>][-d] [--external-dns] <HADOOP_DISTRO>"
    exit 1;
}

# parse arguments
OPTS=`getopt -o r:p:c:d --long "prefix,repository,command:,remote-host:,external-dns" -n 'parse-options' -- "$@"`
...
while true; do
  case "$1" in
    ...
    --remote-host ) REMOTE_HOST="${2}"; shift; shift;;
    -- ) shift; break ;;
    * ) break ;;
  esac
done
...
# start the weave network
net-utils/weave-swarm-network-manager.sh launch ${REMOTE_HOST}

# set weave environment
eval $(weave env)
...
EOM
    ...
    NFS_MOUNTS="--nfs-mounts /opt/hadoop/logs"
fi
...
docker run ${docker_mode} \
    -v ${WORKING_DIR}:/shared ${CLIENT_VOLUMES} \
    -p 2222:22 \
    --dns=${DOCKER_ENVIRONMENT_DNS} \
    --dns=8.8.8.8 \
    --name "${HADOOP_VERSION//.}-client" \
    -e SERVICE_NAME="${HADOOP_VERSION//.}" \
    -e SERVICE_REGION=hadoop \
    ${DOCKERHUB_REPOSITORY_IMAGE_PREFIX}${HADOOP_VERSION} \
    start-container.sh ${docker_cmd_mode} --external-dns ${NFS_MOUNTS} ${COMMAND}
